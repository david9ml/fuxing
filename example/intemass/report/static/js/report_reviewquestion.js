;(function($, undef) {
    ...
    $(function(){
        ...
        var isCtrl = false;
        var editor000 = $('#omitted_editor').ckeditorGet();

        editor000.on('contentDom', function() {
                editor000.document.on('keyup', function(event) {
                if(event.data.$.keyCode == 17) isCtrl = false;
                });
                editor000.document.on('keydown', function(event) {
                if(event.data.$.keyCode == 17) isCtrl = true;
                if(event.data.$.keyCode == 67 && isCtrl == true){
                    try {
    ...
    var loadcanvasbtngroup = function(canvaslist, questionid){
        if (canvaslist['stucanvas']){
            ...
            for (var i = 0; i < canvaslist['stucanvas'].length; i += 1){
                console.warn(canvaslist['stucanvas']);
