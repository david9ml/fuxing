...
from portal.common import getSpById, getTpByRequest, getSpByRequest, getGroupNameByRequest, stripHTMLStrings
...
def student_index(request):
    ...
    try:
        ...
        deadlinewarning = "".join(str for str in _getdeadlinewarning(assignments))
        if deadlinewarning:
...
def _getdeadlinewarning(assignments):
    for assignment in assignments:
        papers = Paper.objects.filter(assignment=assignment)
        for paper in papers:
            questions = Question.objects.filter(paper=paper)
            stuanswers = StudentAnswer.objects.filter(question__in=questions)
            anscount = sum(1 for stuanswer in stuanswers if stuanswer.taked)
    ...
    if studentid:
        try:
            ...
            form = StudentModifyForm(teacher=tp,
                                     initial={"realname": student.user.username,
    ...
    try:
        ...
        for a in assignments:
            ...
            for p in papers:
                ...
                stuanswer = StudentAnswer.objects.filter(student=student,
                                                         question__in=question_set)
                count = sum(1 for sa in stuanswer if sa.taked)
                if count == p.total:
...
def student_takeassignment(request):
    ...
    if paper and questionseq:
        ...
        try:
            question_set = Question.objects.filter(id__in=questionseq)
            stuanswer_set = StudentAnswer.objects.filter(question__in=question_set,
                                                         student=student)
        ...
        if assignment:
            ...
            messages.add_message(request, messages.INFO, 'No question in paper')
    else:
        assignment = None
        qnameseq = []
...
def __initstuanswer(paper, question_set, student):
    ...
    for question in question_set:
        sa = StudentAnswer.objects.get_or_create(student=student, question=question)
    ...
    if saverequest:
        try:
            ...
            question_set = Question.objects.filter(paper=p,
                                                   debugcompleted=Question.ALLCOMPLETED)
            logger.debug(len(question_set))
            stuanswers = StudentAnswer.objects.filter(student=student,
                                                      question__in=question_set)
            logger.debug("you know: %s" % len(stuanswers))
            ...
            print e
            logger.error("can\'t save timeleft")
...
def student_submitanswer(request):
    ...
    try:
        ...
        logger.error("question %s not exists" % question)
        return HttpResponse(simplejson.dumps(response_data), mimetype="application/json")
...
@login_required
def student_gethistoryanswers(request):
    student, res = getSpByRequest(request, 'login')
    if not student and res:
        return res
    ...
    for assignment in assignments:
        ...
        for paper in papers:
            ...
            for q in questions:
                try:
                    stuanswer = StudentAnswer.objects.get(question=q,
                                                          student=student)
                except:
...
def student_submitpaper(request):
    ...
    stuanswers = StudentAnswer.objects.filter(student=student,
                                              question__in=question_set)
    if stuanswers:
        totalmark = sum(ans.mark for ans in stuanswers)
        if totalmark < p.passpoint:
            response_data['state'] = 'failed'
        else:
            for ans in stuanswers:
                ans.taked = True
                ans.save()
...
def __rebasefailedanswer(mark, stuanswers):
    for ans in stuanswers:
        ans.sentencelist = None
        ans.omitted = None
        ans.html_answer = None
        ans.txt_answer = None
        ans.txt_answer = None
        ans.timeleft = -1
        if ans.historymarklist:
            historymark = pickle.loads(str(ans.historymarklist))
        else:
            historymark = []
        mark += ans.mark
        historymark.append(ans.mark)
        ans.historymarklist = pickle.dumps(historymark)
        ans.mark = 0
        ans.stuansimages.all().delete()
        ans.save()
        #delete canvas
        for stucanvas in Canvas.objects.filter(stuanswer=ans):
            stucanvas.delete()
    return mark
...
def student_papersummarize(request):
    ...
    if paper and questionseq:
        ...
        stuanswers = StudentAnswer.objects.filter(student=student,
                                                  question__in=question_set)
        if passed == '0':
            mark = __rebasefailedanswer(mark, stuanswers)
            messages.add_message(request, messages.INFO,
                                 "You failed in this paper, please don\'t be panic, you can take it again")
        else:
            mark = sum(ans.mark for ans in stuanswers)
