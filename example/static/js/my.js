(function ($) {
   ...
   global_legends.clear = function(thiscanvas){
    ...
    Raphael.st.undo = function (x,y) {
        ...
        if (bbox && Raphael.isPointInsideBBox(bbox,x,y)){
            pid = global_legends.remove(thisset);
    ...
    //
     ...
     var buildDrawArea = function(parent, options){
    ...
    var buildToolBar = function(parent, csrfvalue){
        var pid = $(parent).attr('id');
        ...
        function(action, menuele, pos){
            ...
            if (action === "vertical"){
                var drawopts = {'start_x': pos.x, 'start_y': pos.y,
                                ...
                                'custom_path': null, 'custom_handler_path': null,
                                'dashline': true, 'color': colormap[pid],
                                'vertical': true};
                addDraw(pid, drawDot, drawopts);
                var canvas = $(parent).find(".intemasscanvas-canvas")[0];
                $(canvas).unbind("click");
     ...
     *
     *******/
    var addDraw = function(pid, draw_what, drawopts){
        var dialogid = '#'+pid+'-legend-dialog';
        var legend = 'L';
        var dialogOpts = {
            modal: true,
            ...
            open: function(e, ui){
                if(draw_what === drawLine){
                    ...
                    $(e.target).parent().find('label').parent().append(
                                ...
                                "</form>");
                    if(drawopts['vertical']===true) $("#"+pid+"-legend-vertical").attr('checked', true);
                    if(drawopts['horizontal']===true) $("#"+pid+"-legend-horizontal").attr('checked', true);
                }
            },
            close: function(e, ui){
    ...
    var drawComplexCurve = function(pid, drawopts) {
    ...
    var drawCurve = function(pid, drawopts) {
    ...
    var drawLine = function(pid, drawopts){
        var legend = drawopts['legend'];
        var paper = instmap[pid];
        var discattr = {fill: "#00CC00", stroke: "none"};
        var discattr_drag = {fill: "#000", stroke: "none"};
    ...
    var drawDot = function(pid, drawopts) {
        var legend = drawopts['legend'];
        var paper = instmap[pid];
    ...
    var drawTips = function(pid, drawopts) {
        var legend = drawopts['legend'];
        var paper = instmap[pid];
        var start_x = drawopts['start_x'];
        var start_y = drawopts['start_y'];
    ...
    var drawAxis = function(pid, axis){
        clearDeprecatedAxis(pid, axis);
        var paper = instmap[pid];
    ...
    var clearDeprecatedAxis = function(pid, axis){
        ...
        }else if (axis['direction'] === '1' && axismap[pid][1]){
    ...
    var curvecompare3p = function(pid, c1, c2){
    ...
    var curvecompare2p = function(pid, c1, c2){
        if (getcurveintesectpoint(pid, c1, c2, [])){
            return 'Intersect';
        }else{
            var c1_pathbox = getcurvepath(c1);
    ...
    var curvecompare2_3p = function(pid, c1, c2){
        if (getcurveintesectpoint(pid, c1, c2, [])){
            return 'Intersect';
        }else{
            var c1_pathbox = getcurvepath(c1);
    ...
    var getcurverules = function(pid, c1, c2){
        var rules = [];
    ...
    var getpointrules = function(pid, c1, c2){
        var rules = [];
        var rulelang;
    ...
    var getdotrules = function(pid, c1, c2, area){
        var rules = [];
        var dot, curve;
    ...
    var getcurveintesectpoint = function(pid, c1, c2, area){
        var c1_elem = c1['elem'];
        var c2_elem = c2['elem'];
 ...
 //           console.log('start in selectarea');
   ...
   //         console.log('end in selectarea');
    ...
    var getcurveelemlist = function(elset, area){
        switch (elset.getelemtype()){
            case 'complexcurve':
    ...
    var getintesect_set = function(pid, curveelemlist, area){
    ...
    var IntemassCanvasGet = function(thiscanvas, csrfvalue){
    ...
    var IntemassCanvasUpload = function(csrfvalue){
        autogenrulelist();
