...
import pickle
import os
from algo.standard import *
from algo.answer import *
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        rulelist=\
        [{'Mark': 10, 'Point':   ['P1.1', 'P1.2', 'P2', 'P3', 'P4', 'P5', 'P6']},
        ...
        try:
            ...
            plist = [point['Point_No'] for point in pointlist]
            #pointlist, textfdist, slist = sinst.Analysis(stdanswerstring)
        ...
        for i in range(500):
            try:
                file = opendocx(random.choice(filelist))
                anstext = str(getdocumenttext(file)).strip('[]')
                mark,ommited = ans.Analysis(img,anstext,textfdist,slist,pointlist,rulelist)
            except:
